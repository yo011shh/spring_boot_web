...
public class MemberController {
    ...
    public String addmembers(@ModelAttribute @Valid AddMemberRequest request, BindingResult result, Model model) {
    if (result.hasErrors()) {
        model.addAttribute("errors", result.getAllErrors()); // 에러 메시지 전달
        return "join_new"; // 검증 실패 시 회원가입 페이지로 돌아감
    }
    memberService.saveMember(request);
    return "join_end"; // 성공 시
}
...
session.setAttribute("userId", sessionId); // 아이디 이름 설정 // 패스워드 반환
